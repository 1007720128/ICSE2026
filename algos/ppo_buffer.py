...
class GraphReplayBuffer(object):
    ...
    def __init__(self, obs_dim, num_agents, num_services, svc_obs_dim=5):
        ...
        obs_shape = (obs_dim,)
        share_obs_shape = (num_agents * obs_dim,)
        node_obs_shape = (num_agents, obs_dim)
        agent_id_shape = (1,)
        share_agent_id_shape = (num_agents,)
        adj_shape = (num_agents, num_agents)
        ####################
        ...
        self.svc_adj = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, num_services, num_services),
            dtype=np.float32,
        )
    ...
    def insert(self, share_obs: arr, obs: arr, node_obs: arr, adj: arr, agent_id: arr, share_agent_id: arr,
               rnn_states_actor: arr, rnn_states_critic: arr, actions: arr, action_log_probs: arr, value_preds: arr,
               rewards: arr, masks: arr, bad_masks: arr = None, active_masks: arr = None, available_actions: arr = None,
               svc_obs: arr = None, svc_adj=None) -> None:
        """
        ...
        svc_obs: (np.ndarray)
            obs for microservices
            :param svc_adj:
        """
        self.share_obs[self.step + 1] = share_obs.copy()
        self.obs[self.step + 1] = obs.copy()
        self.svc_obs[self.step + 1] = svc_obs.copy()
        self.node_obs[self.step + 1] = node_obs.copy()
        self.adj[self.step + 1] = adj.copy()
        self.svc_adj[self.step + 1] = svc_adj.copy()
    ...
    def after_update(self) -> None:
        ...
        self.svc_obs[0] = self.svc_obs[-1].copy()
        self.node_obs[0] = self.node_obs[-1].copy()
        self.adj[0] = self.adj[-1].copy()
        self.svc_adj[0] = self.svc_adj[-1].copy()
    ...
    def recurrent_generator(
        ...
        """
        # A batch represents a collection of fixed-size data chunks.
        # Each batch contains a fixed number of chunks,
        # and each chunk holds a fixed number of data elements.
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3]
        batch_size = n_rollout_threads * episode_length * num_agents
        ...
        svc_obs = (
            self.svc_obs[:-1]
            .transpose(1, 2, 0, 3, 4)
            .reshape(-1, *self.svc_obs.shape[3:])
        )
        adj = self.adj[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.adj.shape[3:])
        svc_adj = self.svc_adj[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.svc_adj.shape[3:])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                svc_obs_batch.append(svc_obs[ind: ind + data_chunk_length])
                node_obs_batch.append(node_obs[ind: ind + data_chunk_length])
                adj_batch.append(adj[ind: ind + data_chunk_length])
                svc_adj_batch.append(svc_adj[ind: ind + data_chunk_length])
            ...
            svc_obs_batch = np.stack(svc_obs_batch, axis=1)
            node_obs_batch = np.stack(node_obs_batch, axis=1)
            adj_batch = np.stack(adj_batch, axis=1)
            svc_adj_batch = np.stack(svc_adj_batch, axis=1)
            ...
            svc_obs_batch = _flatten(L, N, svc_obs_batch)
            node_obs_batch = _flatten(L, N, node_obs_batch)
            adj_batch = _flatten(L, N, adj_batch)
            svc_adj_batch = _flatten(L, N, svc_adj_batch)
            ...
            yield (share_obs_batch, obs_batch, svc_obs_batch, node_obs_batch, adj_batch, svc_adj_batch,
                   agent_id_batch, share_agent_id_batch, rnn_states_batch, rnn_states_critic_batch,
